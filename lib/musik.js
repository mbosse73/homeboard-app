...
// Bewusst ein Suchlink und kein echter Albumlink: der bräuchte die Spotify-API mitsamt
// Zugangsdaten und Token-Erneuerung. Der Suchlink kostet keine einzige Anfrage, kann deshalb
// auch nie fehlschlagen, und die App öffnet ihn mit dem gesuchten Album als erstem Treffer.
function spotifySucheUrl(interpret, titel) {
  const begriff = titel ? `${interpret} ${titel}` : interpret;
  return `https://open.spotify.com/search/${encodeURIComponent(begriff)}`;
}
...
function alsAlbum(zeile) {
  ...
  if (!teile) {
    ...
    return { interpret: text, titel: null, info: null, url, spotifyUrl: spotifySucheUrl(text, null), coverUrl: null };
  }
  ...
  return { interpret, titel, info, url, spotifyUrl: spotifySucheUrl(interpret, titel), coverUrl: null };
}
